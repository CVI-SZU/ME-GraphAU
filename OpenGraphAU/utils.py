...
def draw_text(path, words):
    import cv2
    # from PIL import Image, ImageDraw, ImageFont
    img = cv2.imread(path)
    pos_y = img.shape[0] // 30
    pos_x  = img.shape[1] // 30
    for i, item in enumerate(words):
        y = pos_y + i * img.shape[0] // 30
        img = cv2.putText(img, str(item), (pos_x, y), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255), 2)
    return img
