...
def swin_transformer_tiny(pretrained=True, **kwargs):
    ...
    if pretrained:
        path_model = os.path.join(models_dir, model_name["swin_transformer_tiny"])
        if not os.path.exists(path_model):
            download_checkpoint(
                "https://github.com/SwinTransformer/storage/releases/download/v1.0.0/swin_tiny_patch4_window7_224.pth"
            )
        model.load_state_dict(torch.load(path_model, map_location=torch.device("cpu"))["model"])
    return model
...
def swin_transformer_small(pretrained=True, **kwargs):
    ...
    if pretrained:
        path_model = os.path.join(models_dir, model_name["swin_transformer_small"])
        if not os.path.exists(path_model):
            download_checkpoint(
...
def swin_transformer_base(pretrained=True, **kwargs):
    ...
    if pretrained:
        path_model = os.path.join(models_dir, model_name["swin_transformer_base"])
        if not os.path.exists(path_model):
            download_checkpoint(
