...
class SCRFD_ONNX:
    def __init__(
        self,
        model_file="saved_weights/face_detection/scrfd_500.onnx",
        providers: list = None,
        input_size: tuple = (320, 320),
    ):
        if providers is None:
            providers = ["CPUExecutionProvider"] if onnxruntime.get_device() == "CPU" else ["CUDAExecutionProvider"]
        self.batched = False
        self.session = onnxruntime.InferenceSession(model_file, None, providers=providers)
        self.center_cache = {}
        self.nms_thresh = 0.4
        self._init_vars()
    ...
    def run(self, img, thresh=0.5, input_size=None, max_num=0, metric="default"):
